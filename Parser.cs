...
/*--------------------------------------------------------------------------*/
	...
	void ProcDecl() {
		string name; Obj obj; int adr; INode fun, t1; 
		Expect(12);
		Ident(out name);
	...
	void Term(out int type, out INode n) {
		...
		while (la.kind == 5 || la.kind == 6) {
			...
			if (type != integer || type1 != integer) SemErr("integer type expected"); else gen.Emit(op); 
		}
	}
